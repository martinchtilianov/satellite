...
try:
    while True:
        ...
        if pos0 and pos1:
            ...
            if disparity > 0:
                focal_length_px = new_K[0, 0] * (display_width / width)  # Adjusted for resized view
                depth = (focal_length_px * baseline) / disparity
                depth_cm = depth * 100  # Convert to centimeters

                # Draw rectangle and distance on view0
                x, y, w, h = rect0
                cv2.rectangle(view0, (x, y), (x + w, y + h), (255, 0, 0), 2)
                cv2.putText(view0, f"{depth_cm:.1f} cm", (x, y - 10),
